          ...
          <div style={{ animation: 'bounce 2s infinite ease-in-out' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', marginBottom: '0.5rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <p style={{ color: 'var(--text-secondary)', fontSize: 'clamp(0.7rem, 2.5vw, 0.9rem)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: '0.4rem' }}>
              ...
              {(() => {
                ...
                const myScore = gd?.scores?.[me.id] || 0;
                const totalQuestions = gd?.questions?.length || 10;
                const percentage = (myScore / (totalQuestions * 10)) * 100;
                
                let grade = 'F';
                let color = 'var(--error)';
                if (percentage >= 100) { grade = 'S'; color = '#fbbf24'; }
                else if (percentage >= 90) { grade = 'A'; color = '#10b981'; }
                else if (percentage >= 80) { grade = 'B'; color = '#3b82f6'; }
                else if (percentage >= 70) { grade = 'C'; color = '#84cc16'; }
                else if (percentage >= 60) { grade = 'D'; color = '#f59e0b'; }
                ...
                return (
                  <div style={{ 
                    ...
                    boxShadow: `0 0 20px ${color}33`, transform: 'rotate(5deg)'
                  }}>
                    <span style={{ fontSize: '0.6rem', fontWeight: 950, color: 'var(--text-secondary)', marginBottom: '-4px' }}>GRADE</span>
                    <span style={{ fontSize: '2.5rem', fontWeight: 950, color: color }}>{grade}</span>
                  </div>
                );
              })()}
            </div>

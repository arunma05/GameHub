...
export async function getLeaderboards(): Promise<Leaderboards> {
  const [wins, results] = await Promise.all([
    // Optimized: Fetch only relevant winCount records (could add limit but let's see)
    prisma.winCount.findMany({
      orderBy: { count: 'desc' },
      take: 100 // Optimization step
    }),
    // Optimized: Fetch only top results
    prisma.result.findMany({
      orderBy: { createdAt: 'desc' },
      take: 500 // Optimization step
    })
  ]);
